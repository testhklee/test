...
<script>
	...
	function list(page){
		location.href="messageTargetList.do?curPage="+page
	}
...
<div class="container">
	<h2 style="margin-top: 50px;"><b style="font-size: 80%; margin-left: 50px; color: #92e3af">받은 쪽지</b></h2>
	...
	<div  align="center">
		<form action="mulDelTarget.do" style="margin-bottom: 200px">
			<table border="1" class="table table-hover table-bordered">
				<tr>
					<th width="10px" style="text-align: center;"><input type="checkbox" onclick="allchk(this.checked);"></th>
					<th width="50px" style="text-align: center;">보낸이</th>
					<th width="700px" style="text-align: center;">내용</th>
					<th width="70px" style="text-align: center;">보낸 날짜</th>
				</tr>
				<c:choose>
					<c:when test="${messageTargetList.size()==0}">
						<tr>
							<td colspan="4" align="center">==========받은 메세지가 없습니다.==========</td>
						</tr>
					</c:when>
					<c:otherwise>
						<c:forEach items="${messageTargetList}" var="messageTargetList">
							<tr>
								<td style="text-align: center;"><input type="checkbox" name="messageNo" value="${messageTargetList.messageNo }"></td>
								<td style="text-align: center;">${messageTargetList.memberName }</td>
								<td><a onclick="window.open('targetOne.do?messageNo=${messageTargetList.messageNo }', '_blank', 'width=370,height=320,left=750, top=250')">${messageTargetList.messageContent }</a></td>
								<td style="text-align: center;">${messageTargetList.messageDate }</td>
							</tr>
						</c:forEach>
					</c:otherwise>
				</c:choose>
			...
			<input class="btn btn-success btn-sm" type="submit" value="삭제">
		</form>
